<div class="row" id="banchay">
  ...
  if(isset($_GET['all'])){
    $result = mysqli_query($conn, "SELECT * FROM product WHERE type = 'Quần' " );
      ...
      $result = mysqli_query($conn, "SELECT * FROM product WHERE price < {$_GET['duoi']} AND type = 'Quần' " );
  }else{
  $result = mysqli_query($conn, "SELECT * FROM product WHERE price >= {$_GET['tren']} AND type = 'Quần' " );
  }
