<div class="row" id="banchay">
  ...
  if(isset($_GET['all'])){
    $result = mysqli_query($conn, "SELECT * FROM product WHERE type = 'Giày' " );
      ...
      $result = mysqli_query($conn, "SELECT * FROM product WHERE price < {$_GET['duoi']} AND type = 'Giày' " );
  }else{
  $result = mysqli_query($conn, "SELECT * FROM product WHERE price >= {$_GET['tren']} AND type = 'Giày' " );
  }
