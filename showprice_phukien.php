<div class="row" id="banchay">
  ...
  if(isset($_GET['all'])){
    $result = mysqli_query($conn, "SELECT * FROM product WHERE type = 'Phụ kiện' " );
      ...
      $result = mysqli_query($conn, "SELECT * FROM product WHERE price < {$_GET['duoi']} AND type = 'Phụ kiện' " );
  }else{
  $result = mysqli_query($conn, "SELECT * FROM product WHERE price >= {$_GET['tren']} AND type = 'Phụ kiện' " );
  }
