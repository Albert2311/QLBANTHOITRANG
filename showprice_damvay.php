<div class="row" id="banchay">
  ...
  if(isset($_GET['all'])){
    $result = mysqli_query($conn, "SELECT * FROM product WHERE type = 'Đầm/váy' " );
      ...
      $result = mysqli_query($conn, "SELECT * FROM product WHERE price < {$_GET['duoi']} AND type = 'Đầm/váy' " );
  }else{
  $result = mysqli_query($conn, "SELECT * FROM product WHERE price >= {$_GET['tren']} AND type = 'Đầm/váy' " );
  }
